...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, input_dim=3, num_classes=10, zero_init_residual=False):
...
class Model(nn.Module):
    def __init__(self, input_dim, num_classes, ):
        ...
        if input_dim != 3:
            self._resnet.conv1 = nn.Conv2d(input_dim, 64, kernel_size=3, stride=1, padding=1,
                                           bias=False)
        if num_classes != 1000:
