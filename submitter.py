...
class CCSubmitter:
    def __init__(self, work_dir="./", stop_on_error=False) -> None:
    ...
    def submit(self, job: str, on_local=False, verbose=False, **kwargs):

        env_script = "\n".join(self._env)

        script = "\n".join(
            [_create_sbatch_prefix(**{**self._sbatch_kwargs, **kwargs}), env_script, job])
        code = self._write_and_run(script, on_local, verbose=verbose)
        if code != 0:
